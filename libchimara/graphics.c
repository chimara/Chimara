...
on_graphics_configure(GtkWidget *widget, GdkEventConfigure *event, winid_t win)
{
	...
	if(needs_resize) {
		...
		cairo_surface_t *new_backing_store = gdk_window_create_similar_surface(gtk_widget_get_window(widget), CAIRO_CONTENT_COLOR, allocation.width, allocation.height);
...
exposed. */
gboolean
on_graphics_expose(GtkWidget *widget, GdkEventExpose *event, winid_t win)
{
